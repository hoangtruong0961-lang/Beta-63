...
export const SchedulePlannerModal: React.FC<SchedulePlannerModalProps> = ({
  ...
  // Load schedule from localStorage on open or perspective change
  useEffect(() => {
    if (isOpen) {
      ...
      const worldId = activeWorld.id || "default";
      const cachedEffects = localStorage.getItem(`ark-butterfly-effects-${worldId}`);
      if (cachedEffects) {
        try {
          const parsed = JSON.parse(cachedEffects);
  ...
  const syncAndSaveSchedule = (updatedDays: parsedDays[], updatedFuture: EventItem[], optThought?: string | null) => {
    ...
    localStorage.setItem(getCacheKey(activePerspective), JSON.stringify(newSchedule));
  };
  ...
  const handleManualSaveSchedule = () => {
    try {
      ...
      localStorage.setItem(getCacheKey(activePerspective), JSON.stringify(fullData));
...
- Viết khách quan, không dùng đại từ nhân xưng như "Tôi" để tránh nhầm lẫn góc nhìn. Dùng đại từ ngôi thứ ba hoặc tên riêng của ${subjectName}.`;
      ...
      localStorage.setItem(getCacheKey(activePerspective), JSON.stringify(scheduleObject));
      setActiveTab(0);
...
${event.npcDynamic && event.npcDynamic !== "Không" ? `Biến động NPC liên đới: ${event.npcDynamic}` : ""}`;
  ...
  const handleClearSchedule = () => {
    if (window.confirm("Bạn có chắc muốn xóa lịch trình hiện tại của góc nhìn này không?")) {
      localStorage.removeItem(getCacheKey(activePerspective));
      setScheduleData(null);
      setRawTextEdit("");
                            ...
                            <button
                              onClick={() => {
                                if (window.confirm("Bạn muốn thiết lập lại toàn bộ và khép lại các vết rạn nứt không-thời gian chứ?")) {
                                  const worldId = activeWorld.id || "default";
                                  localStorage.removeItem(`ark-butterfly-effects-${worldId}`);
                                  setButterflyEffects([]);
                                  setChaosIndex(0);
