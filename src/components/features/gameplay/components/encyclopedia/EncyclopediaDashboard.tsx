    ...
    <div id="encyclopedia-manager-root" className="flex-1 flex flex-col h-full bg-[#020617] text-slate-100 overflow-y-auto custom-scrollbar relative font-sans selection:bg-sky-500/20 selection:text-sky-200">
      ...
      <div className="max-w-6xl mx-auto w-full p-4 lg:p-6 space-y-6 pb-20 relative z-10">
        ...
        <div className="text-center py-6 border-b border-sky-950/40 bg-gradient-to-b from-sky-950/5 to-transparent rounded-2xl p-4">
          ...
          <p className="font-mono text-[10px] text-indigo-400/80 mt-2 tracking-[0.2em] uppercase font-bold">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 bg-[#0d1527]/70 border border-slate-800/80 p-3 rounded-2xl select-none shadow-xl backdrop-blur-md">
          ...
          <div className="flex flex-wrap gap-1.5">
            <button
              ...
              className={`px-3 py-1 text-[10px] font-bold font-mono uppercase rounded-xl border transition-all flex items-center gap-1.5 ${
                showGraph
            ...
            <button
              ...
              className="px-3 py-1 text-[10px] font-bold font-mono uppercase rounded-xl border bg-slate-950 text-sky-400 border-slate-800 hover:border-sky-500/40 hover:text-sky-300 transition-all flex items-center gap-1.5"
            >
        ...
        <div className="bg-[#090f23]/80 border border-sky-950/60 rounded-2xl p-4.5 shadow-xl">
          ...
          <div className="mt-2.5 px-1.5 flex justify-between items-center text-xs font-mono">
            ...
            <span className="text-slate-400 text-[10px]">Đang hiển thị {filteredEntries.length} trong số {entries.length} cổ mẫu</span>
          </div>
        ...
        {(showGraph || showTemplates) && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch animate-fadeIn">
                  ...
                  <div className="flex-1 relative w-full h-[330px] select-none flex items-center justify-center">
                    ...
                    {hoveredNode ? (
                      <div className="absolute right-3.5 top-3.5 w-[220px] bg-slate-950/95 border border-sky-500/20 p-3 rounded-xl shadow-2xl backdrop-blur-md animate-fadeIn text-left space-y-1.5 pointer-events-none">
                        ...
                        <p className="text-[9px] text-slate-300/85 leading-normal line-clamp-3">
            ...
            {showTemplates && (
              <div className={`${showGraph ? "lg:col-span-12 xl:col-span-5" : "lg:col-span-12"} bg-[#090f23]/70 border border-sky-950/50 rounded-2xl p-5 flex flex-col justify-between shadow-2xl relative backdrop-blur-md`}>
        ...
        <div className="space-y-2 text-left">
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-1.5">
              ...
              <h3 className="font-sans text-xs font-bold text-sky-450 uppercase tracking-widest">
            ...
            {activeFilterIndicator && (
              <button 
                ...
                className="text-[10px] font-mono text-sky-450 hover:underline flex items-center gap-1 uppercase font-bold"
              >
                ✕ Hủy lọc nhãn
              </button>
            )}
                ...
                >
                  ...
                  <div className="mt-2 text-ellipsis overflow-hidden z-10 w-full">
                    ...
                    <div className="flex justify-between items-baseline mt-0.5">
                      ...
                      {isSelected && (
                        <span className="text-[8px] font-mono text-sky-400 font-bold">
                          Lọc ⬤
                        </span>
                      )}
                    </div>
                ...
                if (layoutStyle === "dense") {
                  return (
                    <div
                      ...
                      className={`group rounded-xl border text-left transition-all px-4 py-3 shadow-md relative cursor-pointer flex flex-wrap sm:flex-nowrap items-center justify-between gap-3 select-none hover:translate-x-1 ${
                        !isEnabled 
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <div className={`p-1.5 rounded-lg border shrink-0 ${
                          ...
                          {catInfo && React.createElement(catInfo.icon, { size: 14 })}
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <h4 className={`text-xs font-sans font-extrabold tracking-wide text-sky-400 group-hover:text-sky-350 capitalize truncate ${!isEnabled ? 'line-through opacity-50' : ''}`}>
                              {e.keyword || e.title || "Vô danh thư"}
                            </h4>
                            {e.triggerMode === 'always' && <Pin size={9} className="text-sky-400 animate-pulse shrink-0" />}
                            <span className="text-[8px] font-mono text-slate-500 hidden md:inline shrink-0">
                              ~{Math.round((e.text?.length || 0)/3.8)} TK
                            </span>
                          </div>
                          <span className="text-[10px] text-slate-300/80 font-sans truncate block line-clamp-1">
                      ...
                      <div className="flex items-center gap-3 shrink-0 ml-auto sm:ml-0">
                        {e.keywords && e.keywords.length > 0 && (
                          <div className="hidden sm:flex gap-1 overflow-hidden max-w-[140px]">
                            {e.keywords.slice(0, 2).map((w, idx) => (
                              <span key={idx} className="text-[8px] font-mono text-slate-400 bg-slate-950 px-1 py-0.5 rounded truncate max-w-[70px] border border-slate-900/60">
                                #{w}
                              </span>
                            ))}
                          </div>
                        ...
                        {(e.relatedEntries || []).length > 0 && (
                          <span className="text-[8px] font-mono text-sky-450 font-bold hidden sm:inline shrink-0">
                            🔗{(e.relatedEntries || []).length}
                          </span>
                        )}

                        <span className={`px-1.5 py-0.5 rounded text-[8px] border font-bold uppercase shrink-0 ${tierColor}`}>
                          {tierLabel}
                        </span>
