...
const evaluateRuleCondition = (
  ...
  if (!cond || cond.toLowerCase() === 'always' || cond === 'luôn luôn' || cond === 'luôn áp dụng') {
    return true;
  }
...
${backgroundInsights}
  ...
  for (const rule of rulesToEvaluate) {
    const condMatch = rule.match(/^\[(?:ĐIỀU KIỆN|KÍCH HOẠT KHI|COND|CONDITION|TRIGGER):\s*([^\]]+)\]/i);
    if (condMatch) {
      const conditionStr = condMatch[1];
      const isMet = evaluateRuleCondition(conditionStr, turnCount, gameTime, lastUserMessage, tavoChatVars);
      if (isMet) {
        // Strip the condition prefix for cleaner display inside prompt
        const cleanRule = rule.substring(condMatch[0].length).trim();
        activeRules.push(cleanRule);
      }
    } else {
