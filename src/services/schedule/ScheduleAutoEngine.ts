...
export const getButterflyEffects = (worldId: string): ButterflyEffect[] => {
  const cached = localStorage.getItem(getButterflyEffectsKey(worldId));
  if (!cached) return [];
  try {
...
export const saveButterflyEffects = (worldId: string, effects: ButterflyEffect[]): void => {
  localStorage.setItem(getButterflyEffectsKey(worldId), JSON.stringify(effects));
};
...
export const clearButterflyEffects = (worldId: string): void => {
  localStorage.removeItem(getButterflyEffectsKey(worldId));
};
...
export const processScheduleAutoprogression = async (
  ...
  for (const perspective of perspectives) {
    ...
    try {
      const schedule: ScheduleData = JSON.parse(cached);
